...
public:
    TreeNode* travelsal(vector<int>& nums, int left, int right) {
        ...
        res->left = travelsal(nums, left, mid);
        res->right = travelsal(nums, mid + 1, right);
        return res;
    }
    TreeNode* sortedArrayToBST(vector<int>& nums) {
        TreeNode* res = travelsal(nums, 0, nums.size());
        return res;
    }
};
...
int main()
{
    ...
    vector<int> inorder = { -10,-3,0,5,9 }; TreeNode* root1 = treecreate(inorder, 0);
    //vector<int> postorder = { 2,1,3,NULL,4,NULL,7 }; TreeNode* root2 = treecreate(postorder, 0);
    Solution a;
    TreeNode* res = a.sortedArrayToBST(inorder);
    LevelOrder(res);
